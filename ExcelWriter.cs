	...
	class ExcelWriter {
		...
		public bool WriteToExcel() {
			try {
				...
				string folderToSave = Path.Combine(AssemblyDirectory, "Results");
				if (!Directory.Exists(folderToSave))
					Directory.CreateDirectory(folderToSave);

				foreach (SpRecordFileInformation fileInformation in filesInfo.Values) {
					UpdateTextBox("Создание новой книги для: " + fileInformation.WorkstationName);
					string templateFile = Path.Combine(AssemblyDirectory, "Template.xlsx");
					Workbook wb = xlApp.Workbooks.Open(templateFile);
					if (wb == null) {
						xlApp.Quit();
						LoggingSystem.LogMessageToFile("Не удалось открыть книгу: " + templateFile);
						return false;
					}
					...
					float oneLinePercent = oneFilePercentage / (float)fileInformation.DaysInfo.Count;
					int row = 3;
					foreach (KeyValuePair<DateTime, SpRecordFileInformation.DayInfo> dayInfo in fileInformation.DaysInfo) {
						UpdateTextBox("Запись данных: " + dayInfo.Key.ToShortDateString());
						...
						object[] values = new object[] {
							...
							dayInfo.Value.GetMissedCallCount(SpRecordFileInformation.DayInfo.MissedCallType.ConditionalyLostSelf15sec),
							dayInfo.Value.GetMissedCallCount(SpRecordFileInformation.DayInfo.MissedCallType.ConditionalyLostSelf20sec),
							dayInfo.Value.GetMissedCallCount(SpRecordFileInformation.DayInfo.MissedCallType.ConditionalyLostSelf25sec),
							dayInfo.Value.GetMissedCallCount(SpRecordFileInformation.DayInfo.MissedCallType.ConditionalyLostSelf30sec),
							dayInfo.Value.GetMissedCallCount(SpRecordFileInformation.DayInfo.MissedCallType.ConditionalyLostRedirected5sec),
							dayInfo.Value.GetMissedCallCount(SpRecordFileInformation.DayInfo.MissedCallType.ConditionalyLostRedirected10sec),
							dayInfo.Value.GetMissedCallCount(SpRecordFileInformation.DayInfo.MissedCallType.ConditionalyLostRedirected15sec),
							dayInfo.Value.GetMissedCallCount(SpRecordFileInformation.DayInfo.MissedCallType.ConditionalyLostRedirected20sec),
							dayInfo.Value.GetMissedCallCount(SpRecordFileInformation.DayInfo.MissedCallType.ConditionalyLostRedirected25sec),
							dayInfo.Value.GetMissedCallCount(SpRecordFileInformation.DayInfo.MissedCallType.ConditionalyLostRedirected30sec),
							dayInfo.Value.TotalConditionalyLost,
							dayInfo.Value.TotalConditionalyLostPercent
						};
					...
					int columnTotal = 2;
					foreach (string valueTotal in valuesTotal) {
						ws.Cells[row, columnTotal].Formula = valueTotal;
						columnTotal++;
					}
